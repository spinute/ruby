...
文字列全体が必要ない場合には、Ropeに対してそのまま処理を施し、文字列の一部分だけを配列として作成すると、。

このような最適化が可能なメソッドの例としては、先頭の大文字化や部分文字列の取得などがあります。
このためには、Stringクラスのメソッドに対してRopeに対して配列と等価な文字列としての結果を得る処理を実装する必要があります。

TODO: 部分文字列の取得、capitalizeの図の作成
<>

時間の関係上、今回は部分文字列の取得についてのみ実装を行いました。

Ropeの構築のためのメソッドの拡充とも関連してくるのですが、実際のアプリケーションの中で繰り返し、あるいは大きな文字列に対して現れる文字列の構築メソッドと、その構築された文字列に対して適用されるメソッド調査し、それらの効率的なRope実装を行うことは最適化の可能性としてやり残されています。
...
#### 課題
テストケース(make test-all)が手元であと2つだけ原因不明に時々落ちています。
TODO: 落ちているテストケースの明記

結合処理だけではRopeで行うのが圧倒的に早いにもかかわらず、配列表現の結果文字列全体を最終的に必要とするような用途では、Ropeと配列表現の文字列の変換コストによって<<とRope上の+の差がない、という結果になってしまっています。
TODO: ちゃんと書く
2,3つめの最適化について、今回は限られた数のメソッドしか実装することができていませんが、実装したメソッドの他にも最適化が可能なメソッドが多くあり、Ropeのままで大きな文字列を作り、Ropeのままで文字列を小さくしてから最終結果文字列を得られるようなケースがあればRopeを導入しなければ達成できない高速化が達成できたのではないかと考えています。

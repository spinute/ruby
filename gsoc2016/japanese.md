...
参照カウントGCを自力で行う選択をしたことにより、文字列の結合や削除を行う際に木の内部の全てのノードの参照カウントを増減する必要が出てしまい、計算量的には結合O(n)、削除O(n)となってしまっている点は理論的な計算時間に対しての課題です。
...
+によるRopeの構築は結合処理を実行を遅延していると考えることができます。

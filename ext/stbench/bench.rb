...
def bench_delete(keytype, pattern, ht_init_size=0, scale=10, keylen=5)
    puts "Delete bench: %-15s %-15s %-12s %-15s %s" % ["keytype=#{keytype},", "pattern=#{pattern},", "scale=#{scale},", "ht_init_size=#{ht_init_size},", "keylen=#{keylen}"]
    pid = Process.fork do
	bench = STBench.new
	bench.delete_setup keytype, ht_init_size, scale, pattern, keylen
	puts Benchmark.measure { bench.delete_run }
	bench.delete_cleanup
    end
    Process.waitpid pid
end
...
bench_delete 'num', 'same'
bench_delete 'num', 'rand'
bench_delete 'num', 'diff'
bench_delete 'str', 'same'
bench_delete 'str', 'rand'
bench_delete 'str', 'same', 0, 1, 1000
bench_delete 'str', 'rand', 0, 1, 1000
